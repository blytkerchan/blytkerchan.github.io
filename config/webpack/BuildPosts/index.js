...
//const globby = (...args) => depromisify(import('globby').then(({ default: globby }) => globby(...args)));
const rehypeSanitize = (...args) => import('rehype-sanitize').then(({ default: rehypeSanitize }) => rehypeSanitize(...args));
const rehypeStringify = (...args) => import('rehype-stringify').then(({ default: rehypeStringify }) => rehypeStringify(...args));
const remarkParse = (...args) => import('rehype-parse').then(({ default: remarkParse }) => remarkParse(...args));
const remarkRehype = (...args) => import('rehype-hype').then(({ default: remarkRehype } )=> remarkRehype(...args));
const unified = (...args) => import('unified').then(({ default: {unified} } )=> {
  return unified(...args)}
);
...
class BuildPosts {
  ...
  parseInputMarkdown(filename) {
    ...
    if (parserState != ParserState.ERROR) {
      ...
      if (fromDirName[0] !== '/') {
        fromDirName = this.options.paths.appPath + (this.options.paths.appPath[this.options.paths.appPath.length - 1] === '/' ? '' : '/') + fromDirName;
      }
  ...
  async generateFeedXml(RawSource, compilation, posts) {
    ...
    if (Object.hasOwn(config, 'baseUrl')) {
      ...
      if (!URL.canParse(publicUrl)) {
        console.error(`Can't find base URL. Set 'baseUrl' in the config JSON (${configFileName}) -- Not generating feed XML`);
        return;
      }
    }
    ...
    feedContent += `<link href="${publicUrl}${publicUrl[publicUrl.length - 1] === '/' ? '' : '/'}feed.xml" rel="self" type="application/atom+xml"/>`
    ...
    for (const post of posts) {
      ...
      const permalink = (Object.keys(post).includes('permalink')) ? post['permalink'] : `${publicUrl}${publicUrl[publicUrl.length - 1] === '/' ? '' : '/'}blog/${post['date'].replaceAll('-', '/')}/${post['slug']}`;
      ...
      feedContent += `<content type="html" xml:base="${permalink}"><![CDATA[ ${await this.fromMarkdown(post['excerpt'])} ]]></content>`
      feedContent += `</entry>`;
    }
    ...
    compiler.hooks.thisCompilation.tap(pluginName, (compilation) => {
        ...
        (assets) => {
          ...
          filenames.forEach((filename) => {
            ...
            if (post) {
              ...
              compilation.emitAsset(
                `${this.options.to}${this.options.to[this.options.to.length - 1] === '/' ? '' : '/'}${post['filename']}`,
      ...
      compilation.hooks.processAssets.tap(
        ...
        (assets) => {
          ...
          posts = posts.sort((lhs, rhs) => { return rhs['filename'].localeCompare(lhs['filename']) });
